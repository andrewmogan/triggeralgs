...
#define TRACE_NAME "TriggerCandidateMakerPDS"
...
void
TriggerCandidateMakerPDS::operator()(const TriggerActivity& activity, std::vector<TriggerCandidate>& cand)
{ 
  if ((m_activity_count++) % m_prescale == 0)
  {
    TLOG(TLVL_DEBUG_1) << "Emitting PDS TriggerCandidate " << (m_activity_count-1);
    ...
    tc.type = TriggerCandidate::Type::kPDS;
    tc.algorithm = TriggerCandidate::Algorithm::kPDS;
...
void
TriggerCandidateMakerPDS::configure(const nlohmann::json &config)
{
