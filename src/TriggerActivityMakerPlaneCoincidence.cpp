...
void
TriggerActivityMakerPlaneCoincidence::operator()(const TriggerPrimitive& input_tp,
                                                 std::vector<TriggerActivity>& output_ta)
{
  ...
  if (m_induction1_window.is_empty() && isU) {
    m_induction1_window.reset(input_tp);
    m_primitive_count++;
    return;
  }
  if (m_induction2_window.is_empty() && isY) {
    m_induction2_window.reset(input_tp);
    m_primitive_count++;
    return;
  }
  if (m_collection_window.is_empty() && isZ) {
    m_collection_window.reset(input_tp);
    m_primitive_count++;
    return;
  }
  ...
  if (isU && (input_tp.time_start - m_induction1_window.time_start) < m_window_length)
    m_induction1_window.add(input_tp);
  if (isY && (input_tp.time_start - m_induction2_window.time_start) < m_window_length)
    m_induction2_window.add(input_tp);
  if (isZ && (input_tp.time_start - m_collection_window.time_start) < m_window_length)
    m_collection_window.add(input_tp);
  ...
  if (!collectionComplete) {
           ...
           (m_induction1_window.adc_integral + m_induction2_window.adc_integral + m_collection_window.adc_integral) >
             m_adc_threshold &&
    ...
    TLOG(1) << "Emitting low energy trigger with " << m_induction1_window.adc_integral << " U "
            << m_induction2_window.adc_integral << " Y induction ADC sums and " << check_adjacency(m_collection_window)
    ...
    for (auto tp : m_collection_window.inputs)
      dump_tp(tp);
    ...
    if (isZ)
      ...
      m_collection_window.clear();
    if (isU)
      m_induction1_window.reset(input_tp);
    else
      m_induction1_window.clear();
    if (isY)
      m_induction2_window.reset(input_tp);
    else
      m_induction2_window.clear();
  }

  // Otherwise, slide the relevant window along using the current TP.
  else {
    if (isU)
      m_induction1_window.move(input_tp, m_window_length);
    else if (isY)
      m_induction2_window.move(input_tp, m_window_length);
    else if (isZ)
      m_collection_window.move(input_tp, m_window_length);
  }
  m_primitive_count++;
...
TriggerActivityMakerPlaneCoincidence::check_adjacency(TPWindow window) const
{
  ...
  for (size_t i = 0; i < chanList.size(); ++i) {
    ...
    // but also tally up with the tolerance counter.
      ...
      ++adj;
      for (size_t i = 0; i < next_channel - channel; ++i)
        ++tol_count;
    }
