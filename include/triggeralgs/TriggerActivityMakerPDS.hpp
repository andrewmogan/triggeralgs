...
#ifndef TRIGGERALGS_INCLUDE_TRIGGERALGS_TRIGGERACTIVITYMAKERPDS_HPP_
#define TRIGGERALGS_INCLUDE_TRIGGERALGS_TRIGGERACTIVITYMAKERPDS_HPP_
...
{
public:
  virtual ~TriggerActivityMakerPDS() = default;
  void operator()(const TriggerPrimitivePDS& input_tp, std::vector<TriggerActivityPDS>& output_ta)
  {
    // Apply TP filtering
   ...
   */
  virtual void process(const TriggerPrimitivePDS& input_tp, std::vector<TriggerActivityPDS>& output_ta) = 0;
   ...
   */
  virtual bool preprocess(const TriggerPrimitivePDS& input_tp)
  {
   ...
   */
  virtual void postprocess(std::vector<TriggerActivityPDS>& output_ta)
  {
    ...
    if (m_prescale > 1) {
      for (std::vector<TriggerActivityPDS>::iterator iter = output_ta.begin(); iter != output_ta.end();) {
        m_ta_count++;
  ...
  virtual void flush(timestamp_t /* until */, std::vector<TriggerActivityPDS>&) {}
...
#endif // TRIGGERALGS_INCLUDE_TRIGGERALGS_TRIGGERACTIVITYMAKERPDS_HPP_
