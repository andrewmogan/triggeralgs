...
public:
  ...
  {
    if (ch_a.channel == ch_b.channel){
      return ch_a.time_start < ch_b.time_start ; 
    }
    else {
...
private:
  ...
  int time_diff = 30;
  uint16_t braggE = 2000; //  27500 is used in uB based on incoming muon angle vs maxadc
  ...
  int ColPlEndChnl = 960; //from DUNE 2560 for uBooNE 479
  int boxchcnt = 1;
  ...
  int64_t boxwidtime= 4500; // consider not slicing through time     1150
  ...
  std::vector<TriggerPrimitive> master_tp_list;

  std::vector<TriggerPrimitive> tp_list;
  std::vector<TriggerPrimitive> tp_list_cut;
