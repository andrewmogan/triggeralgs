/**
 * @file TriggerCandidateMakerPDS.hpp
 *
...
public:
  ...
  void operator()(const TriggerActivityPDS& input_ta, std::vector<TriggerCandidatePDS>& output_tc)
  {
    // Apply TP filtering
   ...
   */
  virtual void process(const TriggerActivityPDS& input_ta, std::vector<TriggerCandidatePDS>& output_tc) = 0;
   ...
   */
  virtual bool preprocess(const TriggerActivityPDS&)
  {
    return true;
  }
   ...
   */
  virtual void postprocess(std::vector<TriggerCandidatePDS>& output_tc)
  {
    ...
    if (m_prescale > 1) {
      for (std::vector<TriggerCandidatePDS>::iterator iter = output_tc.begin(); iter != output_tc.end();) {
        m_tc_count++;
...
#endif // TRIGGERALGS_INCLUDE_TRIGGERALGS_TRIGGERCANDIDATEMAKERPDS_HPP_
